...
def read_hyperparamaters(path="./src/cache/config.json"):
    ...
    os.system(
        "nvcc --time=build_time.log -DDEBUG -DPARAMS "
        "src/main.cpp src/kernel/utils.cu "
        "src/forward/Kernel/layer_norm.cu src/forward/Kernel/embedding.cu "
        "src/forward/Kernel/linear.cu src/forward/Kernel/attention_head.cu "
        "src/forward/Kernel/interface.cu "
        "src/backpropagation/Kernel/interface_back.cu "
        "src/backpropagation/Kernel/flash_attention.cu "
        "-o src/bin/attention"
    )
